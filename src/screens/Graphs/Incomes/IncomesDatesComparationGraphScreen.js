...
import { apiGetIncomesDateComparation, clearGraphData } from '../../../modules/Graph/GraphActions';
...
class IncomesDatesComparationGraphScreen extends Component
{
    ...
    async _getData()
    {
        try
        {
            ...
            await this.props.apiGetIncomesDateComparation(year, yearTwo, month, monthTwo);

            const allCategories = this._getAllCategories();
            const { dataOne, dataTwo } = this.props.incomes
    ...
    _getAllCategories()
    {
        ...
        for (const key in incomes)
            incomes[key]?.forEach(item => allCategories.push(item.category));
    ...
    formattData(incomes, data, color)
    {
        ...
        {
            dataset.data?.push(income.total)
            if (formattedData?.datasets?.length == 0)
                formattedData?.labels?.push(income.category);
        });
    ...
    render()
    {
        ...
        const chartWidth = data?.length > 4 ? Style.DEVICE_WIDTH * 1.5 * data.labels.length / 8 : Style.DEVICE_WIDTH - 40
        const summaryData = [
            {
                data: data1,
                svg: {
...
export default connect(mapStateToProps, mapStateToPropsAction)(IncomesDatesComparationGraphScreen);
